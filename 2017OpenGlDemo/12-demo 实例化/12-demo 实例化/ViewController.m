...
GLfloat vertices[] = {
    -0.05, -0.05,  0.5,   0.1, 0.8, 0.5,   0.0,0.0,
     0.05, -0.05,  0.5,   0.9, 0.0, 0.4,   1.0,0.0,
    -0.05,  0.05,  0.5,   0.3, 0.6, 0.0,   0.0,1.0,
     0.05,  0.05,  0.5,   0.4, 0.9, 0.5,   1.0,1.0,
    
    -0.05, -0.05, -0.5,   0.5, 0.3, 0.9,   1.0,0.0,
     0.05, -0.05, -0.5,   0.6, 0.2, 0.8,   0.0,0.0,
    -0.05,  0.05, -0.5,   0.7, 0.1, 0.6,   1.0,1.0,
     0.05,  0.05, -0.5,   0.8, 0.4, 0.7,   0.0,1.0,
};
...
- (void)viewDidLoad {
    ...
    GLKVector2 translations[100];
    
    for (GLint i=-10; i<10; i+=2) {
        for (GLint j=-10; j<10; j+=2) {
            GLKVector2 translation;
            translation.x = (GLfloat)i/10.f+offset;
            translation.y = (GLfloat)j/10.f+offset;
            translations[ind++] = translation;
        }
    }
    for (int i=0; i<100; i++) {
        NSString *string =[NSString stringWithFormat:@"offsets[%d]",i];
        glUniform2f(glGetUniformLocation(_myProgram,[string UTF8String]), translations[i].x, translations[i].y);
    }
